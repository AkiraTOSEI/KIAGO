    ...
    with open(file_path, "r") as file:
        for line in file:
            ...
            if len(parts) == 4:
                ...
                print(
                    f"Warning: Skipping line '{line}' as it does not have four columns."
                )

    return np.array(data)


def preprocess_supercon_wtypes(
    FILE_NAME: str, element_table: list[str]
    ...
    assert np.size(supercon_wtypes_vector_formulas, 0) == np.size(
        supercon_wtypes_raw_formulas
    )

    # vector formulas with labels
    supercon_wtypes_vector_formulas_wlabels = np.column_stack(
        (supercon_wtypes_vector_formulas, supercon_wtypes_raw_labels)
    )

    return (
        supercon_wtypes_raw_index,
        supercon_wtypes_vector_formulas,
        supercon_wtypes_raw_tc,
        supercon_wtypes_raw_labels,
        supercon_wtypes_vector_formulas_wlabels,
    )
